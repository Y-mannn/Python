...
def my_func(*params):
    ...
    for i in params:
        count_ += 1
        if i % 2 != 0: #나머지연산자를 사용하여 결과값이 0과 같지 않은 홀수들의 합만 구함
            sum_ += i
    return count_, sum_
#------------------------------------------
...
print('[2-1]가변길이 입력 파라미터의 갯수와 홀수 합은: ', a, ',', b, type(a), type(b)) #a, b 변수 각각은 int
print('[2-2]result 변수 하나로 리턴되는 결과의 값을 받으면: ', result, type(result)) #d이때는 자료형이 tuple
