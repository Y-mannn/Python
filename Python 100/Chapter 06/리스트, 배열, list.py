...
#(3)type() 함수로 리스트의 타입을 출력해보면 list라고 나온다.
#(4)리스트의 요소로는 [10, 3.14, 'a', 'korea'] 등과 같이 정수형, 실수형, 문자형을 하나의 리스트에 담을 수 있다.
#(5)보통 배열은 같은 타입의 데이터 값을 담는 편이나 파이썬의 리스트는 다른 자료형을 담을 수 있다.
#(6)그러나 리스트안에 또 다른 리스트를 요소로써 가질 수는 없다. --> 리스트 안에 또 다른 리스트 요소를 가질 수 있다.
#(7)중복된 값을 가질 수 없다. --> 중복된 값을 가질 수 있다.
#(8)빈 리스트를 생성하는 방법은 a = [] 방식과 a = list() 방식 중 어느 것을 사용해도 무방하다.